...
const CartMenu = () => {
  ...
  return (
    ...
    >
      {/*     MODAL       */}
      <Box className="bg-white fixed r-0 b-0 w-[max(400px, 30%) h-[100%]">
          ...
          <Box>
            {cart.map((item) => (
              <Box key={`${item?.title}-${item?.id}`}>
                <FlexBox p={"15px 0"}>
                  <Box flex={"1 1 40%"}>
                    <img alt={item?.title} width={"123px"} height={"164px"} src={`/uploads/${item?.imgUrl}`} />
                  </Box>
                  <Box flex={"1 1 60%"}>
                    {/* ITEM NAME */}
                    <FlexBox mb={"5px"} style={{ overflow: "hidden", textOverflow: "ellipsis" }}>
                      ...
                      {/* AMOUNT */}
                      <FlexBox m="15px 0">
                        <Box display={"flex"} alignItems={"center"} border={"1.5px solid #A3A7FC"}>
